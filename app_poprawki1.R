                  ...
                  sidebarLayout(
                      sidebarPanel(
                        pickerInput("uczelnia", "Wybierz uczelnię:",
                                    choices = list("Brak wyboru" = "", 
                                                   "Lista uczelni" = unique(ela1$P_NAZWA_UCZELNI)),
                                    choicesOpt = list(
                                      style = c("color: #999", rep("", length(unique(ela1$P_NAZWA_UCZELNI))))
                                    ),
...
server <- function(input, output, session) {
    ...
    observeEvent(input$add_to_comparison, {
        if (input$kierunek != "" &
            ...
            unique_combinations <- comparison_data() %>%
                mutate(kombinacja = paste(kierunek, uczelnia, poziomforma, mediana_lata, sep = ",\n")) %>%
                distinct(kombinacja)
